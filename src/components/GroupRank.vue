<template>
   ...
   <v-row style="margin: 0 3% 0;">
      <v-col cols="12">
         <v-card style="margin-bottom: 50px;">
            <h2 style="margin: 20px 1.5% 0;" class="page-title">群組排名資訊列表</h2>
               ...
               <table style=" width: 97%;">
                  ...
                  <thead>
                     <tr class="table-title">
                        <th class="col1"><strong>項次</strong></th>
                        <td v-if="winwidth" class="col2"><strong>月份/群組</strong></td>
                  ...
                  <tbody v-for="(item, index) in data" :key="index"> 
                     <tr>
                        ...
                        <td v-if="winwidth"><p>{{ item[0] }}</p></td>
                        <td><p>{{ item[1] }}</p></td>
                        <td v-if="winwidth"><p>{{ item[2] }}</p></td>
                        <td v-else><p>{{ item[0] }} / {{ item[2] }}</p></td>
                        <td>
            ...
            <div class="flex-container" style="padding-right: 2px;">
               <h3 class="pageNum">顯示第 1 到 10 項結果，共 {{ datas }} 項</h3>
               <v-row justify="end">
                  <v-pagination :length="pages" :total-visible="5" style="padding-right: 15px;" />
               </v-row>
            </div>
         </v-card>
      </v-col>
